...
NOAA_PMN_DATA_DIR = ROOT / "data" / "processed" / "noaa_pmn"
SOURCE_CANDIDATES = [
    NOAA_PMN_DATA_DIR / "noaa_pmn_all_data_2017_2026_weekly.csv",
    NOAA_PMN_DATA_DIR / "noaa_pmn_all_data_2001_2026.csv",
]
CALHABMAP_GLOB = str(ROOT / "data" / "processed" / "calhabmap" / "*_cleaned.csv")
...
def find_source_csv() -> Path:
    for path in SOURCE_CANDIDATES:
        if path.exists():
            return path
    raise FileNotFoundError("Could not find a NOAA PMN source CSV in data/processed/noaa_pmn/.")
